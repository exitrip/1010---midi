...
MEM_TOP		EQU	4000H	;use for 16KB code memory devices
...
;code memory space for LPC2 with 16KB
;
;block 0, 2KB, 0000h - 07FFh
;block 1, 2KB, 0800h - 0FFFh
;block 2, 2KB, 1000h - 17FFh
;block 3, 2KB, 1800h - 1FFFh
;block 4, 2KB, 2000h - 27FFh
;block 5, 2KB, 2800h - 2FFFh
;block 6, 2KB, 3000h - 37FFh
;block 7, 2KB, 3800h - 3FFFh
...
CSEG	AT	MEM_TOP-512		;Should be 3E00h
...
CSEG	AT	MEM_TOP-256		;Should be 3F00h
...
;	MOV	TL1,@R1		;get baud rate
	...
	MOV	BRGCON,#0	;disable BRG
	MOV	BRGR1,@R1	;get baud rate
	INC	R1		;point to next byte
	MOV	BRGR0,@R1	;get baud rate
	MOV	BRGCON,#03H	;enable BRG & select BRG for baud rate
